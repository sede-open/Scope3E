...
import { SelectScope2Sources, IProps } from '.';
...
const setup = (overrides: Partial<IProps> = {}) => {
  ...
  return render(
    ...
    >
      <SelectScope2Sources {...props} />
    </I18nProvider>
  );
};

describe('SelectScope2Sources', () => {
  const userMock = {
    company: {
      name: 'Company Ltd',
      companySectors: [
  ...
  describe('when next button is clicked', () => {
    it('should track analytics event', async () => {
      ...
      fireEvent.click(
        getByTestId(selectors.getLabelTestId(selectors.electricityInput))
      );
